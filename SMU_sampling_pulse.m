function current_measured = SMU_sampling_pulse(SMU, channel, trig_volt)
    ...
    if( channel == 1 || channel == 2 )
        ...
        fprintf( SMU, [ ':SOUR' ch_str ':PULS:WIDT 1e-3' ]);
        
        %fprintf( SMU, [ ':SENS' ch_str ':FUNC ""CURR""' ] );
        fprintf( SMU, [ ':SENS' ch_str ':FUNC "CURR:DC"' ] ); % enable current measurement
        fprintf( SMU, [ ':SENS' ch_str ':CURR:APER 5e-4' ] ); % Sets the number of power line cycles (NPLC) value instead of setting the integration time for one point measurement.
        ...
        fprintf( SMU, [ ':TRIG' ch_str ':ACQ:DEL 0.5e-3' ] );
